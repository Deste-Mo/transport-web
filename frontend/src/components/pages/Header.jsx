...
export function Header({profileImage}) {
    ...
    const {hideMobileNavigation, setHideMobileNavigation} = useAnimation();
...
const MobileNavigation = ({className, NAV_LINKS, profileImage}) => {
...
const DesktopNavigation = ({className, NAV_LINKS, profileImage}) => {
    const navigate = useNavigate();
    const location = useLocation();
    const {personalInformation} = useAuth();
    return (
...
const Profile = ({profileImage, className}) => {
    ...
    return (
        ...
        >
            ...
            <TemplatePopup setPopupVisible={setTogglePopup} popupVisible={togglePopup}
                           className="fixed top-[86px] right-10" hideOnOutsideClick={false}>
                <OptionItem
                    onClick={() => {
                        setTogglePopup(false);
...
const NavLink = ({
    ...
    return (
        ...
        {number > 0 && (
            <div
                className="absolute max-md:text-small-3 max-md:font-sm bottom-10 -right-2 bg-danger-100 text-white-100 size-[24px] flex items-center justify-center px-2 py-2 rounded-full">
                {number}
            </div>
        )}
                {name}
