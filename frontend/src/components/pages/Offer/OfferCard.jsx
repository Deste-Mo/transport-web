        ...
        <div className="w-full flex items-center justify-between rounded-2xl">
          <div className="flex items-center gap-2">
            <img
              ...
              onClick={() => goToUserProfile(sug.userid)}
              src={image}
            />
            <div className="flex flex-col items-start">
              <p className="text-small-1 font-md hover:underline cursor-pointer" onClick={() => goToUserProfile(sug.userid)}>{sug?.firstname + (!sug?.lastname ? '' : " " + sug?.lastname)}</p>
...
const OfferCardPopup = ({ setPopupVisible, offer }) => {
  ...
  const handleDeletePost = async () => {
    ...
    setMessagePopup("Publication supprimer",TOAST_TYPE.success)
