...
const NavPageLayout = () => {
    ...
    const {savedOffers, getSavedOffers, suggestedOffers, currentUserOffers, getCurrentUserOffers, getSuggestedOffers} = useOffer();
          ...
          <SubHeader name="Sugfestions d'offres" icon="bi bi-robot" />
            {
                suggestedOffers.length > 0 ? (
                    suggestedOffers.map((suggestion) => (<OfferCard detailedProfile={false} key={suggestion.offerid} sug={suggestion} saved={ savedOffers.length > 0 ? savedOffers.find(offer => offer.offerid === suggestion.offerid) : false }/>))
