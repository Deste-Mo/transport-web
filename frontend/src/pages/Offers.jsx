...
const Offers = () => {
    ...
    const { suggestedOffers, savedOffers, getSavedOffers, getSuggestedOffers, getCurrentUserOffers } = useOffer();
    ...
    const filterOffers = () => {
        ...
        return suggestedOffers.length > 0 ? (suggestedOffers.filter((offer) => {
            if (filter === 'all') {
                return true;
            } else if (filter === 'week') {
    ...
    const SearchOffres = suggestedOffers.length > 0 ? (suggestedOffers.filter(suggestion => {
    ...
    return (
        <motion.section className="flex flex-col items-center justify-center w-full gap-6" variants={appVariants} initial="hidden" whileInView="visible" viewport={{once : true}}>
            ...
            <SubHeader name="Offres" icon="bi bi-briefcase-fill" sticky rightContent={suggestedOffers.length > 0 ? <ExpandableSearchBar value={search} setValue={setSearch} /> : null} />
